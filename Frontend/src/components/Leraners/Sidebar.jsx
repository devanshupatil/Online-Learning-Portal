...
const MaterialIcon = memo(({ className }) => (
...
const Sidebar = memo(({ activeSection, onSectionChange, onClose, isMobile = false }) => {
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-lg border border-gray-200 h-full ${isMobile ? 'p-4' : 'p-6'}`}>
      ...
      {/* MY ACCOUNT Section */}
      <div>
        <h3 className={`text-sm font-bold text-gray-500 uppercase tracking-wider ${isMobile ? 'mb-4' : 'mb-3'}`}>MY ACCOUNT</h3>
        <ul className={`${isMobile ? 'space-y-2' : 'space-y-1'}`}>
          {myAccountItems.map((item) => {
            ...
            return (
              <li key={item.id}>
                <button
                  onClick={() => handleSectionChange(item.id)}
                  className={`sidebar-nav-button w-full flex items-center rounded-lg text-left transition-all duration-300 relative overflow-hidden ${
                    isMobile ? 'px-4 py-4 text-base' : 'px-3 py-2 text-sm'
                  } ${
                    isActive
                      ? 'bg-gradient-to-r from-blue-50 to-blue-100 text-blue-700 font-medium border-l-4 border-blue-500 shadow-sm active'

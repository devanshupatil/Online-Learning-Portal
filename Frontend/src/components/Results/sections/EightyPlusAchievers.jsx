...
const EightyPlusAchievers = () => {
  ...
  const classes = useMemo(() => [ALL, ...new Set(ACHIEVERS_80_PLUS.map((a) => a.classKey))], []);
  const streams = useMemo(() => [ALL, ...new Set(ACHIEVERS_80_PLUS.map((a) => a.streamKey))], []);

  const [year, setYear] = useState(ALL);
  const [studentClassKey, setStudentClassKey] = useState(ALL);
  const [streamKey, setStreamKey] = useState(ALL);
  const [range, setRange] = useState(ALL);

  const filtered = ACHIEVERS_80_PLUS.filter((a) => {
    if (year !== ALL && a.year !== year) return false;
    if (studentClassKey !== ALL && a.classKey !== studentClassKey) return false;
    if (streamKey !== ALL && a.streamKey !== streamKey) return false;
      ...
      <div className="max-w-[1280px] mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto mb-10">
          <h2 className="font-display text-[30px] leading-[38px] md:text-[42px] md:leading-[52px] text-on-surface tracking-tight mb-4">
            {t("eightyPlusAchieversTitle")}
          </h2>
          <p className="text-lg text-on-surface-variant">
            {t("eightyPlusAchieversSubtitle")}
          </p>
        </div>
          ...
          <select className={selectClass} value={year} onChange={(e) => setYear(e.target.value)}>
            {years.map((y) => (
              <option key={y} value={y}>
                {y === ALL ? t("eightyPlusAchieversAllYears") : y}
              </option>
            ))}
          </select>
          <select
            className={selectClass}
            value={studentClassKey}
            onChange={(e) => setStudentClassKey(e.target.value)}
          >
            {classes.map((c) => (
              <option key={c} value={c}>
                {c === ALL ? t("eightyPlusAchieversAllClasses") : t(c)}
              </option>
            ))}
          </select>
          <select className={selectClass} value={range} onChange={(e) => setRange(e.target.value)}>
            {RANGES.map((r) => (
              <option key={r} value={r}>
                {r === ALL ? t("eightyPlusAchieversAllPercentages") : r}
              </option>
            ))}
          </select>
          <select className={selectClass} value={streamKey} onChange={(e) => setStreamKey(e.target.value)}>
            {streams.map((s) => (
              <option key={s} value={s}>
                {s === ALL ? t("eightyPlusAchieversAllStreams") : t(s)}
              </option>
            ))}
          </select>
        </div>
        {filtered.length === 0 ? (
          <p className="text-center text-on-surface-variant">{t("eightyPlusAchieversNoResults")}</p>
        ) : (

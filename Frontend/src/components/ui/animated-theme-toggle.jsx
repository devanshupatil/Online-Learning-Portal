...
// Works standalone (uncontrolled) or wired to a real theme source by
// passing `isDark` + `onToggle`, e.g. from the app's ThemeContext.
export const AnimatedThemeToggle = ({ className, isDark: isDarkProp, onToggle, ...props }) => {
  const [internalTheme, setInternalTheme] = useState("light");
  const isControlled = isDarkProp !== undefined;
  const isDark = isControlled ? isDarkProp : internalTheme === "dark";
...
const SolarSwitch = ({ isDark }) => {
  ...
  return (
    <MotionDiv animate={isDark ? "checked" : "unchecked"}>
      <MotionSvg
        width="20"
        height="20"
        viewBox="0 0 25 25"

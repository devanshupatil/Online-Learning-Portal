...
const parentNavItems = [
  { id: "dashboard", labelKey: "parentNavDashboard", icon: "dashboard" },
  { id: "schedule", labelKey: "parentNavSchedule", icon: "calendar_month" },
  { id: "attendance", labelKey: "parentNavAttendance", icon: "event_available" },
  { id: "marks", labelKey: "parentNavMarks", icon: "quiz" },
  { id: "profile", labelKey: "parentNavProfile", icon: "account_circle" },
];

const ParentNavbar = ({ activeSection, onSectionChange, searchQuery, onSearchChange }) => {
        ...
        <div className="flex justify-between items-center h-16 px-4 sm:px-6 lg:px-8 gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            ...
            <div className="relative">
              ...
              >
                <span className="w-8 h-8 rounded-full bg-tertiary-container text-on-tertiary-container flex items-center justify-center text-xs font-bold border border-outline-variant">
                  RS
                </span>
                <span className="text-xs font-semibold text-on-surface hidden lg:block">Rakesh Sharma</span>
              ...
              {profileDropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-surface-container-lowest rounded-xl shadow-lg border border-surface-variant py-2 z-50 animate-in fade-in zoom-in-95 duration-150">
                  <div className="px-4 py-2 border-b border-surface-variant">
                    <p className="text-sm font-bold text-on-surface">Rakesh Sharma</p>
                    <p className="text-xs text-on-surface-variant">{t("parentNavProfileSub", { childName: PARENT_CHILD.name })}</p>
                  </div>
                  <button
      ...
      >
        <div className="flex justify-between items-center h-16 px-6 border-b border-outline-variant">
          ...
          <button
            ...
            onClick={() => setMenuOpen(false)}
            aria-label={t("parentNavCloseMenu")}
          >
            <span className="material-symbols-outlined">close</span>
          </button>
        </div>
        <div className="flex flex-col px-6 py-4 gap-2 overflow-y-auto">
          {parentNavItems.map((item) => renderTab(item, true))}
          <button
            onClick={handleLogout}
